...
import { Component, createElement, options, render } from "preact";
import {
  createEvent,
  getMixedArray,
  mixedArrayHTML,
...
describe("render()", () => {
  ...
  it("should not throw when changing contentEditable to undefined or null", () => {
    ...
    expect(() => render(<p contentEditable={undefined}>foo</p>, scratch)).to.not
      .throw();
    expect(scratch.firstChild.contentEditable).to.equal("inherit");

    expect(() => render(<p contentEditable={null}>foo</p>, scratch)).to.not
      .throw();
    expect(scratch.firstChild.contentEditable).to.equal("inherit");
  });
