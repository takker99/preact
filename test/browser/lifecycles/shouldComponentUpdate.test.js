...
import { Component, createElement, Fragment, render } from "preact";
import { setupScratch, teardown } from "../../_util/helpers";
import { clearLog, logCall } from "../../_util/logCall";
        ...
        <div>
          <table>
            {rows
              ...
              .map((row) => <Row id={row.id} key={row.id} />)}
          </table>
        </div>
      );
    ...
    const App = ({ sortBy }) => (
      <div>
        <table>
          {rows
            .sort((a, b) => (a[sortBy] > b[sortBy] ? -1 : 1))
            .map((row) => <Row key={row.id} id={row.id} a={row.a} b={row.b} />)}
        </table>
      </div>
    );
