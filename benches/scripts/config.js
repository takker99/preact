...
import { benchesRoot, repoRoot, toUrl } from "./utils.js";
...
export const frameworks = [
  ...
  },
  {
    ...
    dependencies: {
      framework: "file:" +
        repoRoot("benches/proxy-packages/preact-master-proxy"),
    },
    async isValid() {
      try {
  ...
  },
  {
    ...
    dependencies: {
      framework: "file:" +
        repoRoot("benches/proxy-packages/preact-local-proxy"),
    },
    isValid() {
      return validateFileDep(this.dependencies.framework);
    },
...
export async function generateConfig(benchPath, options) {
  ...
  for (let framework of frameworksToRun) {
    ...
    if (!(await framework.isValid())) {
      const warnMsg =
        `Could not locate path for ${framework.label}: ${framework.dependencies.framework}. \nSkipping...`;
