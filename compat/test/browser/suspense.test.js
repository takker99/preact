...
class Catcher extends Component {
  ...
  render(props, state) {
    return state.error ? <div>Catcher did catch: {state.error.message}</div> : (
      props.children
    );
  }
}
...
describe("suspense", () => {
  ...
  it("should suspend when a promise is thrown", () => {
    ...
    const FuncWrapper = (props) => <div id="func-wrapper">{props.children}
    </div>;
  ...
  it("should allow multiple sibling children to suspend", () => {
    const [Suspender1, suspend1] = createSuspender(() => <div>Hello first
    </div>);
    const [Suspender2, suspend2] = createSuspender(() => <div>Hello second
    </div>);
  ...
  it("should support suspending multiple times", () => {
    const [Suspender, suspend] = createSuspender(() => <div>initial render
    </div>);
  ...
  it("should allow suspended multiple times", async () => {
    const [Suspender1, suspend1] = createSuspender(() => <div>Suspender 1
    </div>);
    const [Suspender2, suspend2] = createSuspender(() => <div>Suspender 2
    </div>);
